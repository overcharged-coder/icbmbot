...
jobs:
  run-bot:
    concurrency:
      group: bot-run
      cancel-in-progress: false  # Prevent overlap restarts
    ...
    steps:
      ...
      - name: Make Engines + Bot Executable
        run: |
          chmod +x ./BotLi
      ...
      - name: Run Bot
        ...
        run: |
          echo "Starting bot..."
          ./BotLi "matchmaking" "tournament Ym8ublrH the_necrominds" &
          PID=$!

          # Auto-exit after ~345 minutes (so restart runs without overlap)
